...
//rojo el texto de la pregunta, elimina espacios
...
exports.deleteCmd = (socket,rl,id) => {
    ...
    .then((id)=>{
      ...
      log(socket,'Pregunta eliminada '+ id,'red');
    })
    .catch(error => {
      errorlog(socket,error.message);
    })
...
exports.testCmd = (socket,rl,id) => {
  ...
  .then(quiz => {
    ...
    .then(a => {
      if(a == quiz.answer){
        biglog(socket,'Respuesta correcta','green');
      }else{
        biglog(socket,'Respuesta incorrecta','red');
      }
    });
  })
  .catch(error => {
    ...
    const newQuestion = () => {
      ...
      .then(quiz => {
        ...
        .then(a => {
          if(a == quiz.answer){
            puntuacion++;
            log(socket,'CORRECTA. Aciertos: ' + puntuacion);
            ids.splice(id, 1);
            if(ids.length < 1){
            ...
            biglog(socket,'FINAL','red');
            log(socket,'Has conseguido: '+ puntuacion + ' puntos.','magenta');
            rl.prompt();
          }
        });
      });
...
exports.creditsCmd = (socket,rl) => {
    ...
    log(socket,'Jaime Arias Buzunáriz', 'green');
    rl.prompt();
};
